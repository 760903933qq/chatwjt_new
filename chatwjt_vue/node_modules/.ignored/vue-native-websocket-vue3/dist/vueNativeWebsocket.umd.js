(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["vueNativeWebsocket"] = factory();
	else
		root["vueNativeWebsocket"] = factory();
...
// CONCATENATED MODULE: ./src/socket-server/Emitter.ts
  ...
  _createClass(Emitter, [{
    ...
    value: function addListener(label, callback, vm) {
      if (typeof callback === "function") {
        ...
        this.listeners.has(label) || this.listeners.set(label, []); // 向label添加回调函数 | Add callback function to label
...
/* harmony default export */ var socket_server_Emitter = (new Emitter_Emitter());
// CONCATENATED MODULE: ./src/socket-server/Observer.ts
...
var Observer_default = /*#__PURE__*/function () {
  /**
  ...
  function _default(connectionUrl) {
    ...
    this.reconnectTimeoutId = 0; // 重连超时id | Reconnect timeout id

    this.reconnectionCount = 0; // 已重连次数 | Reconnected times
    // 获取参数中的format并将其转成小写 | Get the format in the parameter and convert it to lowercase

    this.format = opts.format && opts.format.toLowerCase(); // 如果url以//开始对其进行处理添加正确的websocket协议前缀 | If the URL starts with // to process it, add the correct websocket protocol prefix
    ...
    this.passToStoreHandler = this.opts.passToStoreHandler; // 建立连接 | establish connection

    this.connect(connectionUrl, opts); // 如果配置参数中有传store就将store赋值 | If store is passed in the configuration parameters, store will be assigned
  ...
  _createClass(_default, [{
    ...
    value: function connect(connectionUrl) {
      ...
      var protocol = opts.protocol || ""; // 如果没传协议就建立一个正常的websocket连接否则就创建带协议的websocket连接 | If no protocol is passed, establish a normal websocket connection, otherwise, create a websocket connection with protocol

      this.WebSocket = opts.WebSocket || (protocol === "" ? new WebSocket(connectionUrl) : new WebSocket(connectionUrl, protocol)); // 启用json发送 | Enable json sending
    ...
    value: function reconnect() {
      ...
      if (this.reconnectionCount <= this.reconnectionAttempts) {
        this.reconnectionCount++; // 清理上一次重连时的定时器 | Clear the timer of the last reconnection

        window.clearTimeout(this.reconnectTimeoutId); // 开始重连
        ...
        this.reconnectTimeoutId = window.setTimeout(function () {
          ...
          _this2.connect(_this2.connectionUrl, _this2.opts); // 触发WebSocket事件 | Trigger Web Socket events
    ...
    value: function onEvent() {
      ...
      ["onmessage", "onclose", "onerror", "onopen"].forEach(function (eventType) {
        ...
        _this3.WebSocket[eventType] = function (event) {
          socket_server_Emitter.emit(eventType, event); // 调用vuex中对应的方法 | Call the corresponding method in vuex
          ...
          if (_this3.reconnection && eventType === "onopen") {
            ...
            _this3.opts.$setInstance && _this3.opts.$setInstance(event.currentTarget); // 清空重连次数 | Empty reconnection times
    ...
    value: function defaultPassToStore(eventName, event) {
      ...
      var method = "commit"; // 事件名称字母转大写 | Turn the letter of the event name to uppercase

      var target = eventName.toUpperCase(); // 消息内容 | Message content

      var msg = event; // data存在且数据为json格式 | data exists and the data is in json format

      if (this.format === "json" && event.data) {
        // 将data从json字符串转为json对象 | Convert data from json string to json object
        msg = JSON.parse(event.data); // 判断msg是同步还是异步 | Determine whether msg is synchronous or asynchronous
  ...
  install: function install(app, connection) {
    ...
    if (opts.connectManually) {
      app.config.globalProperties.$connect = function () {
        ...
        connectionOpts.$setInstance = opts.$setInstance; // 创建Observer建立websocket连接 | Create Observer to establish websocket connection

        observer = new Observer_default(connectionUrl, connectionOpts); // 全局添加$socket | Add $socket globally
      ...
      app.config.globalProperties.$disconnect = function () {
        if (observer && observer.reconnection) {
          ...
          observer.reconnection = false; // 移除重新连接计时器 | Remove the reconnection timer
      ...
      observer = new Observer_default(connection, opts); // 全局添加$socket属性，连接至websocket服务器 | Add the $socket attribute globally to connect to the websocket server
        ...
        if (hasProxy) {
          this.$options.sockets = new Proxy({}, {
            set: function set(target, key, value) {
              // 添加监听 | Add monitor
              socket_server_Emitter.addListener(key, value, vm);
              target[key] = value;
              return true;
            },
            deleteProperty: function deleteProperty(target, key) {
              // 移除监听 | Remove monitor
              socket_server_Emitter.removeListener(key, vm.$options.sockets[key], vm);
              delete target.key;
              return true;
            }
          });
          ...
          if (sockets) {
            Object.keys(sockets).forEach(function (key) {
              // 添加监听 | Add monitor
              socket_server_Emitter.addListener(key, sockets[key], vm);
            });
          }
        }
      },
...
//# sourceMappingURL=vueNativeWebsocket.umd.js.map